...
def edge_info(dataset, args):
    ...
    hop_edge_index, hop_edge_att = ILD_decomposition(dataset.data.x.size(0), args.ild_layer, adjlist, dataset.data.edge_index)
    torch.save(hop_edge_index, './ild_info/hop_edge_index_' + args.dataset + '_' + str(args.ild_layer))
    torch.save(hop_edge_att, './ild_info/hop_edge_att_' + args.dataset + '_' + str(args.ild_layer))
